...
public static class quad{
    ...
    public static double ClenshawCurtis(Func< double, double> f,
        double a,
        double b,
        double acc=0.01,
        double eps=0.01,
        double f2=NaN,
        double f3=NaN){
            Func<double,double> f_subst;

            if(a==-1 && b==1){
                f_subst = x => f(Cos(x))*Sin(x);
            }
            else{
                f_subst = x =>f( (a+b)/2 + (b-a)/2 * Cos(x) ) * Sin(x)*(b-a)/2;
            }
            return integrate(f_subst,0,PI);
        }
